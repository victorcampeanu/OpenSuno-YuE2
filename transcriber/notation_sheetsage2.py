...
def _notes_to_arr(notes, subbeat_times, voice_id, diagnostics=None):
    ...
    for note in sorted(notes, key=lambda item: (item.start, item.end, item.pitch)):
        ...
        if end_t <= start_t:
            if (diagnostics is not None and
                    subbeat_times[0] <= note.start < note.end <= subbeat_times[-1]):
                diagnostics.append(
                    f"{voice_id}: omitted sub-grid note pitch={note.pitch} at "
                    f"{note.start:.6f}-{note.end:.6f} from ABC; original MIDI retained")
                continue
            raise MelodyVoiceError(
...
def repair_isolated_missing_beats(beats):
    """Interpolate one skipped beat only when both neighboring intervals agree.

    Keep original events untouched; ambiguous grids still fail validation.
    """
    repaired, diagnostics = [], []
    for i, left in enumerate(beats):
        repaired.append(left)
        if i == 0 or i + 2 >= len(beats):
            continue
        before, right, after = beats[i - 1], beats[i + 1], beats[i + 2]
        meter = (left.declared_numerator, left.denominator)
        if any((b.declared_numerator, b.denominator) != meter for b in (before, right, after)):
            continue
        if right.beat_id != left.beat_id + 2 or right.beat_id > meter[0]:
            continue
        if left.beat_id != before.beat_id % meter[0] + 1 or after.beat_id != right.beat_id % meter[0] + 1:
            continue
        half_gap = (right.time - left.time) / 2
        if half_gap <= 0 or any(abs(period - half_gap) > half_gap * .15
                                for period in (left.time - before.time, after.time - right.time)):
            continue
        missing = replace(left, time=left.time + half_gap, beat_id=left.beat_id + 1)
        repaired.append(missing)
        diagnostics.append(f"Interpolated missing beat {missing.beat_id} at {missing.time:.3f}s between beat rows {left.line_no}-{right.line_no}")
    return repaired, diagnostics


def _assemble_abc_score(midi, beats, keys, structures, chords, *, meter_conflict, melody_only):
    beats, repairs = repair_isolated_missing_beats(beats)
    ...
    for voice_id in VOICE_IDS:
        ...
        voice_arrs[voice_id] = _notes_to_arr(notes, subbeat_times, voice_id, diagnostics)
