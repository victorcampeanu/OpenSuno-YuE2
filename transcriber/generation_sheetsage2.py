...
def constrained_prompt_generate_batch(
    ...
    decoder_device = getattr(model, "decoder_device", audio.device)
    ...
    # Move the encoder memory once so an offloaded decoder does not copy it per step.
    memory = memory.to(decoder_device)
    current_length = len(prefix)
    while active_sample_ids and current_length < int(max_sequence_length):
        with inference_autocast(decoder_device, autocast_dtype):
        ...
        keep = torch.tensor(keep_positions, dtype=torch.long, device=decoder_device)
