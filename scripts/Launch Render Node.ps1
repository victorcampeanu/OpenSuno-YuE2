...
if (Test-Path -LiteralPath '.cuda-venv\Scripts\python.exe') {
    & '.\.cuda-venv\Scripts\python.exe' 'app\check_cuda_runtime.py'
...
if (-not $env:OPENSUNO_NODE_TOKEN) {
    ...
    Write-Host '  $env:OPENSUNO_NODE_TOKEN = "choose-a-secret"; .\scripts\"Launch Render Node.ps1"'
}
...
& '.\.venv\Scripts\python.exe' 'app\render_node.py'
